        ...
        tokenHandler.verify(req.header('Authorization').split(' ')[1])
            .then(id => {
                try {
                    ...
                    var total_num = fs.readdirSync(`./faces/${req.race}/training/level-7`).length;
                    var random_index = Math.floor(Math.random() * total_num );
                    var data = fs.readFileSync(`./faces/${req.race}/training/level-7/${random_index}.jpg`);
                    images.push(new Buffer(data, 'binary').toString('base64'));
                    //random choose 7 pictures from Level-X
                    var total_num = fs.readdirSync(`./faces/${req.race}/training/level-${req.body.level - 1}`).length;
                    ...
                    for (var i = 0; i < 7; i++) {
                        ...
                        var data = fs.readFileSync(`./faces/${req.race}/training/level-${req.body.level - 1}/${random_index}.jpg`);
                        images.push(new Buffer(data, 'binary').toString('base64'));
                    }
                    // -------Debug session codes-------
                    // var images = [];
            ...
            .then(id => {
                try {
                    ...
                    for (var i = 0; i < 8; i++) { // Generate 8 random numbers between 0 and 30
                        ...
                        faceNums.push(face);
                        var data = fs.readFileSync(`./faces/${req.race}/daily-assessment/${faceNums[i]}.jpg`);
                        images.push(new Buffer(data, 'binary').toString('base64'));
                    }
                    res.status(200).send({images: images});
                } catch (err) {
        ...
        tokenHandler.verify(req.header('Authorization').split(' ')[1])
            .then(id => {
                try {
                    var images = [];
                    for (var i = 0; i < 30; i++) {
                        var data = fs.readFileSync(`./faces/${req.race}/pre-post-assessment/${i}.jpg`);
                        images.push(new Buffer(data, 'binary').toString('base64'));
                    }
                    res.status(200).send({images: images});
                } catch (err) {
            ...
            .then(id => {
                try {
                    ...
                    for (var i = 0; i < afterFaces; i++) {
                        var data = fs.readFileSync(`./faces/${req.race}/training/level-${req.body.level + 1 - 1}/${i}.jpg`);
                        images.push(new Buffer(data, 'binary').toString('base64'));
                    }
